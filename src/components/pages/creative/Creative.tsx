...
import { Gallery } from '../../header/shared/selectedGallery';
    ...
    return (
            ...
            {pM && <div className='relative w-screen h-full bg-black overflow-y-hidden'>
                <div
                    className={` absolute top-0 left-0 right-0 bottom-[60px] bg-black`}>
                        <Gallery imageList={selectedImages.map(id => `${process.env.PUBLIC_URL}/images/photography/${id}.jpg`)} />
                    </div>
                {phoneFilters && (
            ...
            {!pM && (
                <div className="flex flex-row w-full h-full">
                    ...
                    <Gallery imageList={selectedImages.slice(0, selectedImages.length/2).map(id => `${process.env.PUBLIC_URL}/images/photography/${id}.jpg`)} />
                    </div>
                    <div className='w-1/3 h-full bg-black'>
                    <Gallery imageList={selectedImages.slice(selectedImages.length/2).map(id => `${process.env.PUBLIC_URL}/images/photography/${id}.jpg`)} />
                    </div>
                </div>
            )}
        </>
