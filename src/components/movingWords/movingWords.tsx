...
export function MovingWords({ children }: { children: React.ReactNode }) {
    ...
    const eventToggler = useRef(false); //allow anything to move only when it is on
    const wordHolder = useRef<HTMLDivElement>(null); //pointer to the words
    const originalWord = useRef<string>(""); //used to return the context to original form at the end or inturrupted 
